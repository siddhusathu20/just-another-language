...
public class Parser {
    ...
    Statement parseClass() {
        ...
        if (next(TokenType.INHERITS)) {
            consume(TokenType.IDENTIFIER, "Expected name of superclass");
            superclass = new Expression.Variable(prev());
        }
        ...
        return new Statement.Class(name, methods, superclass);
    }
    ...
    Expression parseAssignment() {
        ...
        if (next(TokenType.EQ)) {
            ...
            if (expr instanceof Expression.Variable) {
                ...
                Expression.Index index = (Expression.Index) expr;
                return new Expression.SetIndex(index.object, index.bracket, index.index, value);
            }
    ...
    Expression parseFactor() {
        ...
        while (next(TokenType.ASTERISK, TokenType.SLASH, TokenType.PERCENT)) {
            Token operator = prev();
    ...
    Expression parseFuncCall() {
        ...
        while (true) {
            if (next(TokenType.L_BRACKET)) {
                expr = call(expr);
            } else if (next(TokenType.L_SQ_BR)) {
                expr = array(expr);
    ...
    Expression parseArray() {
        ...
        if(!check(TokenType.R_SQ_BR)) {
            do {
                elements.add(parseExpression());
            } while (next(TokenType.COMMA));
        }
        Token bracket = consume(TokenType.R_SQ_BR, "Expected ] at end of array");
    ...
    Expression parsePrimary() {
        ...
        if (next(TokenType.SUPER)) {
            Token keyword = prev();
            consume(TokenType.DOT, "Expected '.' after 'super'");
            Token method = consume(TokenType.IDENTIFIER, "Expected identifier");
    ...
    Expression array(Expression arr) {
        ...
        Token bracket = consume(TokenType.R_SQ_BR, "Expected ] after index");
        return new Expression.Index(arr, bracket, index);
    }
