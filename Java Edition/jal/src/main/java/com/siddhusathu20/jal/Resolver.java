...
public class Resolver implements Expression.Visitor<Void>, Statement.Visitor<Void> {
    ...
    public Void visit(Statement.Class stmt) {
        ...
        if (stmt.superclass != null) {
            if (stmt.name.lexeme.equals(stmt.superclass.name.lexeme))
                Main.error(stmt.name.line, "A class cannot inherit from itself");
    ...
    public Void visit(Expression.Index expr) {
        resolve(expr.object);
        resolve(expr.index);
        return null;
    }

    public Void visit(Expression.SetIndex expr) {
        resolve(expr.object);
        resolve(expr.index);
        resolve(expr.value);
        return null;
    ...
    public Void visit(Expression.Super expr) {
        if (currentClass == ClassType.MAIN) {
            Main.error(expr.keyword.line, "Can't refer to 'super' outside a class");
            return null;
        }
        if (currentClass != ClassType.SUBCLASS) {
            Main.error(expr.keyword.line, "Can't refer to 'super' in a class with no superclass");
            return null;
        }
        resolveLocal(expr, expr.keyword);
        return null;
